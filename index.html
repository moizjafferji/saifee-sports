...
(function addAdminQr(){
  ...
  document.getElementById('btnRotateQr').onclick = ()=> issueAndRender(10);
    ...
    function pageCreateOrEdit(edit){
      ...
      const saveBtn = $('#f_save');
if (saveBtn) saveBtn.onclick = (ev)=> saveProfile(!!edit, ev.currentTarget);
    ...
    function saveProfile(edit, btn){
  function setErr(m){ $('#f_err').textContent = m || ''; }

  // Gather & validate
  const data = {
    whatsapp:  $('#f_whatsapp').value.trim(),
    its:       $('#f_its').value.trim(),
    email:     $('#f_email').value.trim(),
    first_name:$('#f_first').value.trim(),
    last_name: $('#f_last').value.trim(),
    height_in: $('#f_height').value,
    weight_lb: $('#f_weight').value.trim(),
    dob:       $('#f_dob').value,
    emer_name: $('#f_emer_name').value.trim(),
    emer_phone:$('#f_emer_phone').value.trim(),
    jamaat:    $('#f_jamaat').value
  };

  const err = validateProfile(data, !edit);
  if (err) { setErr(err); return; }

  const file = $('#f_pic').files[0];
  const key  = edit
    ? `profile:update:${(state.user && state.user.its) || data.its}`
    : `profile:create:${data.its}`;

  withClickLock(key, btn, edit ? 'Saving…' : 'Creating…', (done)=>{
    // Finalize after optional upload
    const finalize = (picUrl)=>{
      const payload = picUrl ? Object.assign({}, data, { profile_pic_url: picUrl }) : data;

      if (!edit){
        google.script.run
          .withSuccessHandler(function(r){
            done();
            if (!r || !r.ok){ setErr((r && r.error) || 'Create failed'); return; }
            state.user = Object.assign({}, data, { profile_pic_url: picUrl });
            setPage('home');
          })
          .withFailureHandler(function(e){
            done();
          ...
          .api_createProfile(payload);
      } else {
        google.script.run
          .withSuccessHandler(function(r){
            done();
            if (!r || !r.ok){ setErr((r && r.error) || 'Save failed'); return; }
            if (picUrl) state.user.profile_pic_url = picUrl;
          ...
          .api_updateProfile(state.user.its, payload);
      }
    };
    ...
    if (file){
      try{
        ...
        fr.onload = function(){
          google.script.run
            .withSuccessHandler(function(u){ finalize(u && u.url); })
            .withFailureHandler(function(e){ done(); setErr((e && e.message) || 'Upload failed'); })
            .api_uploadProfilePic(fr.result, file.name, edit ? state.user.its : data.its);
        };
        fr.onerror = function(){ done(); setErr('Could not read image.'); };
        fr.readAsDataURL(file);
      } catch(_){
        done(); setErr('Could not read image.');
      }
    } else {
      finalize(null);
    }
